...
- **linux** [amd64](https://github.com/ryarnyah/kafka-offset/releases/download/0.0.6/kafka-offset-linux-amd64)
...
#### Running with Docker
```bash
docker run ryarnyah/kafka-offset:0.0.6 <option>
```
...
##### Example
```bash
docker run ryarnyah/kafka-offset:0.0.6 -sink log -source-brokers localhost:9092
```
...
##### Example
```bash
docker run ryarnyah/kafka-offset:0.0.6 -sink kafka -source-brokers localhost:9092 -kafka-sink-brokers localhost:9092 -kafka-sink-topic metrics
```
...
##### Example
```bash
docker run ryarnyah/kafka-offset:0.0.6 -sink elasticsearch -source-brokers localhost:9092 -elasticsearch-sink-url localhost:9200 -elasticsearch-sink-index metrics
```
